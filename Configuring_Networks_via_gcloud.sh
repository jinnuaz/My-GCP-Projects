...
gcloud compute instances create pnet-vm --zone="us-west1-a" --machine-type=n1-standard-1 --subnet=private-sub
gcloud compute instances create lnet-vm --zone="us-west1-a" --machine-type=n1-standard-1 --subnet=labnet-sub

# Now list all the VM instances (sorted by zone):
gcloud compute instances list --sort-by=us-west1-a